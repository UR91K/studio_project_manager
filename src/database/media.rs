...
impl LiveSetDatabase {
    ...
    pub fn insert_media_file(&mut self, media_file: &MediaFile) -> Result<(), DatabaseError> {
        debug!(
            ...
            media_file.original_filename, media_file.id
        );
    ...
    pub fn delete_media_file(&mut self, file_id: &str) -> Result<(), DatabaseError> {
        ...
        let rows_affected = self
            .conn
            .execute("DELETE FROM media_files WHERE id = ?", params![file_id])?;
    ...
    pub fn update_collection_cover_art(
        &mut self,
        collection_id: &str,
        cover_art_id: Option<&str>,
    ) -> Result<(), DatabaseError> {
        debug!(
            "Updating collection {} cover art to: {:?}",
            collection_id, cover_art_id
        );
        ...
        if rows_affected > 0 {
            info!(
                "Successfully updated collection cover art: {}",
                collection_id
            );
        } else {
    ...
    pub fn update_project_audio_file(
        &mut self,
        project_id: &str,
        audio_file_id: Option<&str>,
    ) -> Result<(), DatabaseError> {
        debug!(
            "Updating project {} audio file to: {:?}",
            project_id, audio_file_id
        );
    ...
    pub fn list_media_files(
        ...
        let media_files = stmt.query_map([], |row| self.row_to_media_file(row))?;
    ...
    pub fn get_media_files_by_type(
        &self,
        media_type: &str,
        limit: Option<i32>,
        offset: Option<i32>,
        ...
        let mut stmt = self.conn.prepare(&query)?;
        let media_files = stmt.query_map([media_type], |row| self.row_to_media_file(row))?;
    ...
    pub fn get_orphaned_media_files(
        &self,
        limit: Option<i32>,
        offset: Option<i32>,
    ) -> Result<Vec<MediaFile>, DatabaseError> {
    ...
    pub fn get_media_statistics(&self) -> Result<(i32, i64, i32, i32, i32, i64), DatabaseError> {
        ...
        let (total_files, total_size): (i32, i64) =
            stmt.query_row([], |row| Ok((row.get(0)?, row.get(1)?)))?;

        // Get cover art count
        let mut stmt = self
            .conn
            .prepare("SELECT COUNT(*) FROM media_files WHERE media_type = 'cover_art'")?;
        let cover_art_count: i32 = stmt.query_row([], |row| row.get(0))?;

        // Get audio file count
        let mut stmt = self
            .conn
            .prepare("SELECT COUNT(*) FROM media_files WHERE media_type = 'audio_file'")?;
        ...
        let (orphaned_count, orphaned_size): (i32, i64) =
            stmt.query_row([], |row| Ok((row.get(0)?, row.get(1)?)))?;
    ...
    pub fn get_media_files_count_by_type(&self, media_type: &str) -> Result<i32, DatabaseError> {
        let mut stmt = self
            .conn
            .prepare("SELECT COUNT(*) FROM media_files WHERE media_type = ?")?;
    ...
    pub fn get_project_audio_file(
        ...
        if let Some(ref file) = media_file {
            debug!(
                "Found audio file for project {}: {}",
                project_id, file.original_filename
            );
        } else {
    ...
    pub fn get_collection_cover_art(
        &self,
        collection_id: &str,
    ) -> Result<Option<MediaFile>, DatabaseError> {
        ...
        if let Some(ref file) = media_file {
            debug!(
                "Found cover art for collection {}: {}",
                collection_id, file.original_filename
            );
        } else {
