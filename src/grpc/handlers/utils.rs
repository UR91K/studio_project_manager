...
pub fn convert_live_set_to_proto(live_set: LiveSet, db: &mut LiveSetDatabase) -> Result<Project, DatabaseError> {
    ...
    let tasks = db.get_project_tasks(&project_id)?
        .into_iter()
        .map(|(task_id, description, completed, created_at)| crate::grpc::proto::Task {
            id: task_id,
            description,
            completed,
