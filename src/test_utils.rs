...
fn generate_dev_identifier(plugin_format: PluginFormat) -> String {
...
fn generate_mock_plugin(index: usize) -> Plugin {
...
pub(crate) fn generate_mock_live_set(index: usize) -> LiveSet {
...
fn generate_test_live_sets_vec(count: usize) -> Vec<LiveSet> {
    (0..count).map(generate_mock_live_set).collect()
}

pub(crate) fn generate_test_live_sets_arc(count: usize) -> Arc<Vec<LiveSet>> {
