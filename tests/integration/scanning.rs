...
fn test_process_projects_integration() {
    ...
    assert!(
        !expected_projects.is_empty(),
        "No projects found in configured paths"
    );
...
fn test_process_projects_with_progress() {
    ...
    let progress_updates =
        std::sync::Arc::new(std::sync::Mutex::new(
            Vec::<(u32, u32, f32, String, String)>::new(),
        ));
    ...
    assert!(
        result.is_ok(),
        "process_projects_with_progress failed: {:?}",
        result.err()
    );
    ...
    let phases: Vec<String> = updates
        .iter()
        .map(|(_, _, _, _, phase)| phase.clone())
        .collect();
    println!("Received phases: {:?}", phases);

    // Should have at least starting and completed phases
    assert!(
        phases.contains(&"starting".to_string()),
        "Missing 'starting' phase"
    );
    assert!(
        phases.contains(&"completed".to_string()) || phases.contains(&"preprocessing".to_string()),
    ...
    for (i, (completed, total, progress, _, _)) in updates.iter().enumerate() {
        ...
        assert!(
            *progress >= 0.0 && *progress <= 1.0,
            "Progress out of range at update {}: {}",
            i,
            progress
        );
        ...
        if *total > 0 {
            assert!(
                *completed <= *total,
                "Completed {} exceeds total {} at update {}",
                completed,
                total,
                i
            );
    ...
    println!(
        "✅ Progress streaming test passed with {} updates",
        updates.len()
    );
}
