...
async fn test_get_statistics_with_trace_logging() {
    ...
    match get_stats_resp {
        Ok(response) => {
            ...
            println!("✅ Statistics endpoint succeeded!");
            ...
            println!("🏷️  Total tags: {}", stats.total_tags);
        ...
        Err(e) => {
            println!("❌ Statistics endpoint failed with error: {:?}", e);
...
async fn test_get_statistics_with_empty_database() {
    ...
    match get_stats_resp {
        Ok(response) => {
            let stats = response.into_inner();
            println!("✅ Statistics endpoint succeeded with empty database!");
            println!("📊 Total projects: {}", stats.total_projects);
...
async fn test_individual_statistics_functions() {
    ...
    // Test individual statistics functions that might be causing issues
    {
        ...
        match db_guard.get_projects_per_year() {
            Ok(projects_per_year) => {
                println!("✅ get_projects_per_year succeeded: {:?}", projects_per_year);
            }
            Err(e) => {
                println!("❌ get_projects_per_year failed: {:?}", e);
        ...
        match db_guard.get_projects_per_month(12) {
            Ok(projects_per_month) => {
                println!("✅ get_projects_per_month succeeded: {:?}", projects_per_month);
            }
            Err(e) => {
                println!("❌ get_projects_per_month failed: {:?}", e);
        ...
        match db_guard.get_recent_activity(30) {
            Ok(recent_activity) => {
                println!("✅ get_recent_activity succeeded: {:?}", recent_activity);
            }
            Err(e) => {
                println!("❌ get_recent_activity failed: {:?}", e);
    ...
    // Test the task completion trends function specifically
    {
        ...
        match db_guard.get_task_completion_trends(12) {
            Ok(task_trends) => {
                println!("✅ get_task_completion_trends succeeded: {:?}", task_trends);
            }
            Err(e) => {
                println!("❌ get_task_completion_trends failed: {:?}", e);
